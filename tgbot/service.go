...
func ShowService(bot *tgbotapi.BotAPI, db *sql.DB, chatID int64) {
	log.InfoLogger.Printf("(ShowService) User %d", chatID)
	err := ServiceMsg(bot, chatID)
	if err != nil {
		log.ErrorLogger.Fatalf("(ServiceMsg) %v", err)
	}
...
func HandlerService(bot *tgbotapi.BotAPI, db *sql.DB, update tgbotapi.Update, user *database.Context) {
	...
	log.InfoLogger.Printf("(HandlerService) User %d", chatID)
	...
	if err != nil {
		log.ErrorLogger.Fatalf("(CtxUpdateUserService) %v", err)
	}
